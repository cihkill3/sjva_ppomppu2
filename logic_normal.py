...
class LogicNormal(object):
    ...
    def process_insert_feed():
        ...
        if is_rss:
            ...
            for url in rss_url:
                include_hot = ModelSetting.get_bool('include_hot')
                if include_hot:
                    datas +=LogicNormal.get_crawl('https://www.ppomppu.co.kr/zboard/zboard.php?id=' + LogicNormal.get_board_ids(url)[0] + '&hotlist_flag=999')
                    logger.debug('https://www.ppomppu.co.kr/zboard/zboard.php?id=' + LogicNormal.get_board_ids(url)[0] + '&hotlist_flag=999')
                else:
